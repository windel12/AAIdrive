...
class MapAppService: CarAppService() {
	...
	override fun onCarStart() {
		if (appSettings[AppSettings.KEYS.ENABLED_GMAPS].toBoolean()) {
			...
			val mapController = GMapsController(applicationContext, MapResultsSender(applicationContext), virtualDisplay, MutableAppSettingsReceiver(applicationContext, null /* specifically main thread */))
